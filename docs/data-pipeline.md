...
Cập nhật 25/08/2026. Tài liệu này trả lời ba câu: **đã bổ sung gì vào parquet**,
...
| --- | ---: | ---: | --- |
| `frames.parquet` | 177.321 | 21 | manifest chính — mỗi dòng một keyframe |
...
Một bộ shot-boundary công khai, phủ đúng **873/873** video.
...
Một bộ ASR công khai (**Apache-2.0**) phủ đúng **873/873**, kèm cả bản đã
...
### Caption VLM — bỏ được 27–49 giờ suy luận

Một bộ công khai (**Apache-2.0**) sinh bằng VLM tiếng Việt, mô tả từng keyframe
bằng văn xuôi.

Kiểm chứng trước khi nhận, giống cách đã làm với shot:

```
phủ keyframe của ta          97,4%  (172.684 / 177.321)
caption phân biệt trong video  trung vị 100%, không video nào < 50%
khớp OCR của ta              32,6% vs 12,3% nếu lấy caption frame
                             ngẫu nhiên — gấp 2,7×
```

Chỉ số cuối vừa xác nhận caption mô tả đúng frame, vừa xác nhận phép nối theo
`(video_id, keyframe_n)` đúng — nối sai thì phải rơi về mức ngẫu nhiên. Phép
kiểm tra "phân biệt trong video" loại được kiểu hỏng phổ biến nhất của caption
sinh hàng loạt: mọi frame bị mô tả giống nhau.

Kèm theo là `ocr_text_vlm` — chữ VLM **trích trong ngoặc kép**. Nó đáng giá vì
VLM đọc chữ Việt tốt hơn EasyOCR rõ rệt (`CẢNH BÁO SẠT LỞ NGUY HIỂM` so với
`CẢMH BÁO SẠT LỎ`), nên được thêm **cạnh** `ocr_text` chứ không thay thế.

### ⚠ Đã loại một bộ caption khác

Một bộ caption tiếng Anh (MIT) phủ 873/873 video và lấp đủ 30 video
nguồn chính thiếu, nhưng đo cùng cách chỉ **gấp 2,1×** (5,7% tuyệt đối). Đã đo lại
bằng token trung lập ngôn ngữ để không thiệt cho tiếng Anh — vẫn thấp. Cột
`frame_idx` của họ chỉ khớp **2.309/177.321** với `original_frame_id` của ta.

Chưa chứng minh được là lệch frame, nhưng đủ để không đưa vào index.
...
| Việc | Lý do |
| --- | --- |
| Caption VLM tự chạy | **Không cần nữa** — 97,4% đã mượn được từ nguồn ngoài (Apache-2.0), thay cho 27–49 giờ suy luận. Xem mục 3 |
